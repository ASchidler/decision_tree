...
def _init_vars(instance, num_nodes, c_var):    
    ...
    for i in range(1, instance.num_features + 1):
        ...
        data.d0.append([])
        data.d1.append([])
        data.a[i].append(None)
        data.u[i].append(None)
        data.d0[i].append(None)
        data.d1[i].append(None)
        for j in range(1, num_nodes + 1):
            data.a[i].append(data.pool.id(f"a{i}_{j}"))
            data.u[i].append(data.pool.id(f"u{i}_{j}"))
            data.d0[i].append(data.pool.id(f"d0_{i}_{j}"))
            data.d1[i].append(data.pool.id(f"d1_{i}_{j}"))
...
def _encode_examples(data, instance, num_nodes, solver, start=0):
    for ie in range(start, len(instance.examples)):
        ...
        for j in range(1, num_nodes + 1):
            ...
            for r in range(1, instance.num_features + 1):
                clause.append(data.d0[r][j] if e.features[r] else data.d1[r][j])
...
def _decode(model, instance, num_nodes, data):
    ...
    _check_consistency(data, model, num_nodes, tree)
    return tree


def _check_consistency(data, model, num_nodes, tree):
