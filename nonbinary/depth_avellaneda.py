...
def _alg1(instance, e_idx, limit, lvl, q, clause, fs, x, slv, t):
    ...
    for f in range(1, instance.num_features + 1):
        slv.add(z3.Or([*clause, z3.Not(x[e_idx][lvl]), int(example.features[f]) <= t[q], z3.Not(fs[q][f])]))
    ...
    for f in range(1, instance.num_features + 1):
        slv.add(z3.Or([*clause, x[e_idx][lvl], int(example.features[f]) > t[q], z3.Not(fs[q][f])]))
