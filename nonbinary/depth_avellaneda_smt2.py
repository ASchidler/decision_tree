    ...
    for i in range(1, 2**limit):
        ...
        for cf in range(1, instance.num_features + 1):
            ...
            for j in range(0, len(instance.domains[cf]) - (0 if cf in instance.is_categorical else 1)):
                f[i][instance.feature_idx[cf] + j] = z3.Bool(f"f{i}_{instance.feature_idx[cf] + j}")
        ...
        for c_n in range(0, 2 ** limit):
            ...
            for c1, c1v in c[c_n].items():
                ...
                for c2, c2v in c[c_n].items():
                    if c2 > c1:
                        slv.add(slv.Or([z3.Not(c1v), z3.Not(c2v)]))
...
def _alg1(instance, e_idx, limit, lvl, q, clause, fs, x, slv):
    if lvl == limit:
        return
    ...
    for f in range(1, instance.num_features + 1):
        ...
        for i2 in range(0, len(instance.domains[f]) - (0 if f in instance.is_categorical else 1)):
            ...
            if (not is_cat and c_val > instance.domains[f][i2]) or (is_cat and c_val != instance.domains[f][i2]):
                slv.add(z3.Or([*clause, z3.Not(x[e_idx][lvl]), z3.Not(fs[q][base_idx + i2])]))
            if (not is_cat and c_val <= instance.domains[f][i2]) or (is_cat and c_val == instance.domains[f][i2]):
                slv.add(z3.Or([*clause, x[e_idx][lvl], z3.Not(fs[q][base_idx + i2])]))

    clause.append(z3.Not(x[e_idx][lvl]))
    _alg1(instance, e_idx, limit, lvl+1, 2 * q + 1, clause, fs, x, slv)
    clause.pop()

    clause.append(x[e_idx][lvl])
    _alg1(instance, e_idx, limit, lvl+1, 2*q, clause, fs, x, slv)
    clause.pop()
    ...
    for i in range(1, num_leafs):
        ...
        for f, fv in fs[i].items():
            if bool(model[fv]):
                if f_found:
                    raise RuntimeError(
                        f"ERROR: double features found for node {i}, features {f} and {tree.nodes[i].feature}")
                f_found = True
                real_f = None
...
def encode_impurity_limit(vs, solver, dl):
    c = vs["c"]
    cm = vs["class_map"]
